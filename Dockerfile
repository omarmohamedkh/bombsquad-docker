...
RUN if [[ "${TARGETPLATFORM}" = "amd64" ]]; then \
    wget https://files.ballistica.net/bombsquad/builds/BombSquad_Server_Linux_x86_64_${BOMBSQUAD_VERSION}.tar.gz -O- | tar -xzvf - ; \
    elif [[ "${TARGETPLATFORM}" = "arm64" ]]; then \
    wget https://files.ballistica.net/bombsquad/builds/BombSquad_Server_Linux_Arm64_${BOMBSQUAD_VERSION}.tar.gz -O- | tar -xzvf - ; \
    else \
